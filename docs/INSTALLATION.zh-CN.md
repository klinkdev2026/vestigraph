...
Vestigraph 需要 Python 3.10 或更新版本、KLayout 桌面 0.30.x，以及 `klayout-klink>=0.6.0,<0.7`。正常 `vestigraph` 包会安装兼容 Klink 依赖。Python `klayout` 包用于离线预览；它不会安装 KLayout 桌面应用。

Klink 负责 KLayout 插件安装。首次安装和升级都使用 `klink plugin install`。

## PyPI 发布后的安装

首个公开版本尚未发布到 PyPI。在发布前，请使用下面的 [GitHub Actions release artifacts](https://github.com/klinkdev2026/vestigraph/actions/workflows/release.yml)。发布后可按包名安装。
...
重启运行 `klink-mcp` 的 MCP 客户端。Klink 扩展注册表会在该 Python 环境中发现 Vestigraph 并登记本地 companion。不需要单独的 Vestigraph MCP server。
...
在两个项目都发布到 PyPI 之前，下载由匹配的 Klink 与 Vestigraph 修订构建的 artifacts。把两个 Klink Rust wheel、`klayout_klink` core wheel 和 Vestigraph wheel 放入同一个本地目录，然后运行：
