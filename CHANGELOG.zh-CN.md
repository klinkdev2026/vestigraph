...
- 将一次连续的 AI 版图操作合并为一个历史检查点，并记录有界的操作原因和方法。
- AI 修改前先保存待处理的 KLayout 手动修改，使手动工作保留在历史中。
- Agent 默认只读取最近 30 条检查点摘要；只在用户明确要求时读取全部历史。
- 检查点摘要增加修改时间、保存时间、操作记录、覆盖范围和恢复来源。
- 改进未保存版图的引导，普通历史功能无需开启实验性技能精炼。
- 兼容旧版 Klink 插件，并在新版插件上使用同步事件刷新。
